...
namespace NorthWind.UI.Controllers
{
    public class OrderDetailController : Controller
    {
        ...
        private readonly string _baseUrl = "http://localhost:5155/api/OrderDetail";

        public OrderDetailController(IHttpClientFactory factory, IHttpContextAccessor accessor)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (!res.IsSuccessStatusCode) return View(new List<OrderDetailDto>());
            var json = await res.Content.ReadAsStringAsync();
            var data = JsonConvert.DeserializeObject<List<OrderDetailDto>>(json);
            return View(data);
        }
