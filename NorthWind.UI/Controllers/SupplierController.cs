...
namespace NorthWind.UI.Controllers
{
    public class SupplierController : Controller
    {
        ...
        private readonly string _baseUrl = "http://localhost:5155/api/Supplier";

        public SupplierController(IHttpClientFactory factory, IHttpContextAccessor accessor)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (!res.IsSuccessStatusCode) return View(new List<SupplierDto>());
            var json = await res.Content.ReadAsStringAsync();
            var data = JsonConvert.DeserializeObject<List<SupplierDto>>(json);
            return View(data);
        }
