...
namespace NorthWind.UI.Controllers
{
    public class ShipperController : Controller
    {
        ...
        private readonly string _baseUrl = "http://localhost:5155/api/Shipper";

        public ShipperController(IHttpClientFactory factory, IHttpContextAccessor accessor)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (!res.IsSuccessStatusCode) return View(new List<ShipperDto>());
            var json = await res.Content.ReadAsStringAsync();
            var data = JsonConvert.DeserializeObject<List<ShipperDto>>(json);
            return View(data);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ShipperDto model)
        {
            AddAuth();
